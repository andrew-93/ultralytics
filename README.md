# Экспорт модели (.pt --> .onnx) для YOLOv8 Instance Segmentation:

Для конвертирования можно использовать докер `ultralytics/yolov5`  

* настраиваем параметры в `default.yaml`:
```
topk_all: 100
bhwc_for_det_masks: True
```
где:  
`topk_all` - максимальное количество детекций на выходе детектора,  
`bhwc_for_det_masks` - порядок следования значений элементов на выходе `det_masks`.
Для запуска инференса в trtx_ros поставить значение `True`. 
В этом случае поменяется последовательность значений элементов на выходе `det_masks` с дефолтного BCHW на BHWC. 
При этом размерность выхода `det_masks` не поменяется (останется такой же, как было в BCHW). 

* запускаем конвертирование:
```bash
python3 yolov8_onnxtrt.py -w yolov8x-seg.pt -s 1024x512
```
`-w`, `--weigths` - путь к исходным весам (файл .pt);  
`-s`, `--input_size` - желаемое входное разрешение.   
Результат конвертирования (файл .onnx) будет сохранен по тому же пути, где хранится исходный файл .pt  
.
